...
- **Bot challenges (reCAPTCHA / Cloudflare, etc.)**: When `download_literature_pdf.py` detects a bot interstitial, stop automated fetch and ask the user to place the PDF at `_downloads/{slug}.pdf` (no retry loops on the same URL). See `pdf-paper-ingestion` Step 1b.
