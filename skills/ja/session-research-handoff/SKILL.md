...
| フィールド | 形式 | 例 |
| :--- | :--- | :--- |
| **Agent** | 実行環境（IDE/CLI） | `Cursor`, `Claude Code` |
| **Model** | `プロバイダ / モデル名 / バージョン` | `Moonshot AI / Kimi / K3` |

- **記録前**: Step 2 実行環境同定チェックリストを完了する（自己認識→UI→ユーザー確認、推測禁止）。
- **scholarly-agent-skills のみのコミット**にも同様に付与する（論文プロジェクトの再現性のため）。

### §5 セッションログ（例外記録のみ）

`docs/<paper-id>/design/test-cases.md` §5 は、以下**のみ**に使用する（コミットごとの行追加は不要）:

- **コミットしない作業**（外部レビュー、著者の口頭申告、対話のみの設計レビュー等）
- **遡及補完**（プロトコル策定前セッション等、commit message に Agent/Model が存在しない履歴）
- **セッション要約**（複数コミットにまたがる narrative を1行で残す場合・任意）

> v2.3.0 の「コミットしたら §5 に行追加」は **v2.4.0 で廃止**（ハッシュ追記コミットの連鎖を防ぐため）。
...
**コミット message** への `Agent:` / `Model:` 記載、または §5 への例外行記載の**前に**、以下を必ず実行する。**直前のログ行・直前コミット message からコピーしてはならない**（同一チャット内でもモデル切替があり得る）。
