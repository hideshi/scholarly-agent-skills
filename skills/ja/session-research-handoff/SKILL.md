...
## 不変条件（v2.3.0）: コミットしたらログに書く

> **Invariant**: **Git コミットを作成したら、必ず `docs/<paper-id>/design/test-cases.md` §5 セッションログに行を追加する。** セッション終了を待たない。

| タイミング | 必須アクション |
| :--- | :--- |
| **コミット前**（推奨） | Step 2 実行環境同定チェックリストを完了し、§5 に追記する行の内容を確定する |
| **コミット後** | 確定したコミットハッシュを §5 の「関連コミット」列に記載する |

- 同一セッションで複数コミットした場合: **モデル・作業内容が同じなら1行にハッシュを追記**、モデル切替または作業テーマが変わったら**新しい行**を追加する。
- スキルリポジトリ（`scholarly-agent-skills`）のみのコミットでも、論文プロジェクトの §5 に関連コミットとして記録する。
